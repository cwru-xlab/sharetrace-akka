...
def _max(name: str) -> Expr:
    return pl.col(name).max()


def _sum(name: str) -> Expr:
...
def _percentile(name: str, value: float, interpolation: str = "midpoint") -> Expr:
...
def test_runtime_normality(df: DF, test: Callable[[NDArray], Any]) -> dict[str, Any]:
