...
abstract class BaseSampler<T> implements Sampler<T> {
  ...
  protected static double normalizedSample(RealDistribution distribution) {
    double max = boundedUpperSupport(distribution);
    double min = boundedLowerSupport(distribution);
    return (boundedSample(distribution) - min) / (max - min);
  }

  protected static double boundedUpperSupport(RealDistribution distribution) {
    return Math.min(Double.MAX_VALUE, distribution.getSupportUpperBound());
  }

  protected static double boundedLowerSupport(RealDistribution distribution) {
    return Math.max(Double.MIN_VALUE, distribution.getSupportLowerBound());
  }

  protected static double boundedSample(RealDistribution distribution) {
    return Math.max(Double.MIN_VALUE, Math.min(Double.MAX_VALUE, distribution.sample()));
  }
